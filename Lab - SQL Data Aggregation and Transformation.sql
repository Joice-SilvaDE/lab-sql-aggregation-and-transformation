...
select round((avg(length)/60),0) as avg_duration_hrs
from film;
-- use floor
...
select DATEDIFF(max(rental_date), MIN(return_date)) as rental_days
from rental;
...
-- To achieve this, you need to retrieve the concatenated first and last names of customers, along with the first 3 characters of their email address, so that you can 
-- address them by their first name and use their email address to send personalized recommendations. The results should be ordered by last name in ascending order to make it 
...
select rating, count(*) as nr_films
from film
group by rating;
